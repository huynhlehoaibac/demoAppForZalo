...
public class CustomOAuth2AccessTokenResponseHttpMessageConverter
  ...
  public CustomOAuth2AccessTokenResponseHttpMessageConverter() {
    super(DEFAULT_CHARSET, MediaType.APPLICATION_JSON, new MediaType("text", "json"));
  }
  ...
  protected OAuth2AccessTokenResponse readInternal(Class<? extends OAuth2AccessTokenResponse> clazz,
    ...
    try {
      ...
      Map<String, String> tokenResponseParameters = (Map<String, String>) this.jsonMessageConverter
  ...
  protected void writeInternal(OAuth2AccessTokenResponse tokenResponse,
    ...
    try {
      ...
      this.jsonMessageConverter.write(tokenResponseParameters,
          PARAMETERIZED_RESPONSE_TYPE.getType(), MediaType.APPLICATION_JSON, outputMessage);
    } catch (Exception ex) {
  ...
  /**
   * Sets the {@link Converter} used for converting the {@link OAuth2AccessTokenResponse} to a
   * {@code Map} representation of the OAuth 2.0 Access Token Response parameters.
   *
   * @param tokenResponseParametersConverter the {@link Converter} used for converting to a
   *        {@code Map} representation of the Access Token Response parameters
   */
  public final void setTokenResponseParametersConverter(
    ...
    Assert.notNull(tokenResponseParametersConverter,
        "tokenResponseParametersConverter cannot be null");
  ...
  private static class OAuth2AccessTokenResponseConverter
    ...
    private static final Set<String> TOKEN_RESPONSE_PARAMETER_NAMES =
        new HashSet<>(Arrays.asList(OAuth2ParameterNames.ACCESS_TOKEN,
            OAuth2ParameterNames.TOKEN_TYPE, OAuth2ParameterNames.EXPIRES_IN,
...
//      }
  ...
  private static class OAuth2AccessTokenResponseParametersConverter
    ...
    public Map<String, String> convert(OAuth2AccessTokenResponse tokenResponse) {
      ...
      parameters.put(OAuth2ParameterNames.ACCESS_TOKEN,
          tokenResponse.getAccessToken().getTokenValue());
      parameters.put(OAuth2ParameterNames.TOKEN_TYPE,
      ...
      if (!CollectionUtils.isEmpty(tokenResponse.getAccessToken().getScopes())) {
        parameters.put(OAuth2ParameterNames.SCOPE, StringUtils
            .collectionToDelimitedString(tokenResponse.getAccessToken().getScopes(), " "));
      }
      if (tokenResponse.getRefreshToken() != null) {
        parameters.put(OAuth2ParameterNames.REFRESH_TOKEN,
